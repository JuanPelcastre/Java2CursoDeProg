...
public class CFecha
{
  ...
  public CFecha(int... fecha)
  {
    ...
    if (!fechaCorrecta())
    {
      System.out.println("Fecha incorrecta. Se asigna la predeterminada.");
  ...
  public void asignarFecha(int... fecha)
  {
    ...
    if (fecha.length > 0) día = fecha[0]; else día = fa[0];
    if (fecha.length > 1) mes = fecha[1]; else mes = fa[1];
    if (fecha.length > 2) año = fecha[2]; else año = fa[2];
  }
